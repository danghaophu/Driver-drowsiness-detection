...
sounds = {
    'eye': (r'E:\do an tot nghiep\SafeDriveVision-master\SafeDriveVision-master\eye.mp3', 10),
    'regarder': (r'E:\do an tot nghiep\SafeDriveVision-master\SafeDriveVision-master\regarder.mp3', 10),
    'reposer': (r'E:\do an tot nghiep\SafeDriveVision-master\SafeDriveVision-master\reposer.mp3', 15),
    'phone': (r'E:\do an tot nghiep\SafeDriveVision-master\SafeDriveVision-master\phone.mp3', 15),
    'welcome': (r'E:\do an tot nghiep\SafeDriveVision-master\SafeDriveVision-master\s1.mp3', 0),
    'welcome_eng': (r'E:\do an tot nghiep\SafeDriveVision-master\SafeDriveVision-master\welcomeengl.mp3', 0),
    'WELCOME': (r'E:\do an tot nghiep\SafeDriveVision-master\SafeDriveVision-master\WELCOME.mp3', 0)
}
...
while True:
    ...
    if len(faces)==0 : 
        cv2.putText(img, "Nguoi lai xe khong nhin ve phia truoc", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    ...
    for detection in detections:
        if int(detection[5]) == 67:  # 67 est l'index de 'cell phone'
            ...
            cv2.putText(img, f'Đien thoai {conf:.2f}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            print("Tài xế đang sử dụng điện thoại ", current_time)
            COUNTER2 += 1
            if COUNTER2 >= 3:
                 
                cv2.putText(img, "Hay cat dien thoai", (x1, y1 - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
                sound_thread("phone")
                COUNTER2 = 0
    for face in faces:
...
'''
        # Dessiner les points de repère
            ...
            cv2.putText(img, f'Ngoc đau: {head_angle:.2f}',(10,70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
            size = img.shape
            frame_height = img.shape[0]
            head_tilt_degree, start_point, end_point, end_point_alt = getHeadTiltAndCoords(size, image_points, frame_height)

            cv2.putText(img, f'Nghien đau: {head_tilt_degree[0]:.2f} degrees', (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
            # cv2.line(img, start_point, end_point, (0, 255, 0), 2)
            if 75 > head_angle and head_angle > 110 :
                cv2.putText(img, "Hay nhin ve phia truoc", (x, y - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                COUNTER3 += 1
                if COUNTER3 >= 6:  
                    sound_thread("regarder")          
                    cv2.putText(img, "Hay nhin ve phia truoc", (x, y - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            ...
            if ear < 0.3:
                        cv2.putText(img, "Mat nham", (x, y - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                        COUNTER1 += 1
                        if COUNTER1 >= 4:
                            sound_thread("mat")
                            repeat_counter += 1
                            ...
                            if repeat_counter >= 3:
                                ...
                                repeat_counter = 0
                                cv2.putText(img, "Mat nham 3 lan", (x, y - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            else:
                        COUNTER1 = 0
                        repeat_counter = 0
            if mar > 0.4:
                        sound_thread("reposer")
                        cv2.putText(img, "Ngap", (x, y - 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            head_tilt_degree, start_point, end_point, end_point_alt = getHeadTiltAndCoords(size, image_points, frame_height)
